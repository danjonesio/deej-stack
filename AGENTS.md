...
| Validate | `claude plugin validate .` | open **Customize → Skills** and confirm `d-plan` is listed; **Customize → Hooks** lists one `sessionStart` and one `beforeShellExecution` |
...
| Session start | `SessionStart`, matcher on the source (`startup`); plain stdout becomes context, and every hook's stdout is added | `sessionStart`, fire-and-forget; stdout must be `{"additional_context": "…"}`. When several hooks answer one event Cursor keeps only the last `additional_context`, so there is one entry, `hooks/cursor-session-start.py`, which runs every session-start script and joins what they return. A new session-start script goes in its `SCRIPTS` list and in `hooks.json`, never in a second Cursor entry |
