...
| Branch protection | the default branch has no active ruleset requiring a pull request, or the checks its ruleset requires no longer match the checks that run on every PR, or the ask says `review` | [`references/branch-protection.md`](references/branch-protection.md) |
