...
Every panel member is an Opus sub-agent (`model: "opus"`) spawned with the Agent tool, `subagent_type: "general-purpose"`, and a `name` from the tables below. Build each prompt from [`reviewer-prompt.md`](reviewer-prompt.md). Members never edit files; they read, search, and report.
...
| `security-analyst` | resumed via SendMessage | Adversarial pass over the concrete steps: is every wave-1 requirement mapped to a step, and does any new step open a hole the threat model missed? |
| `reuse-scout` | resumed via SendMessage | Dedup pass: does any step re-implement something in the reuse inventory, or add an abstraction beside an existing one? |
| situational analysts | resumed via SendMessage | Each re-checks the draft through its own lens. |

Resume a wave-1 member rather than spawning a new one; it already holds the codebase context it built. Spawn fresh only when the resume fails.
