...
| situational analysts | when the plan's **Panel record** ran them | `data-analyst`, `ux-api-designer`, `ops-analyst`, `perf-analyst`: each checks that the requirements it raised in the plan landed in the diff, through its own lens from `../../plan/references/panel.md`. |
